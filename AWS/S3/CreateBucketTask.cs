...
namespace S3Operations
{
    class CreateBucketTask
    {
        ...
        async Task VerifyBucket(IAmazonS3 s3Client, string bucketName)
        {
            ...
            bool buckectExists = await AmazonS3Util.DoesS3BucketExistV2Async(s3Client, bucketName);

            while (!buckectExists)
            {
                System.Threading.Thread.Sleep(500);
                buckectExists = await AmazonS3Util.DoesS3BucketExistV2Async(s3Client, bucketName);
            }
